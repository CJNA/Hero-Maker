...
namespace AIBehavior
{
	...
	public class AIBehaviors : AIComponent
	{
		...
		public AIBehaviorExamples.PlayerStats playerStat;
		public float applyAgilityOffset;
		...
		void Start()
		{
			...
			playerStat = GetComponent<AIBehaviorExamples.PlayerStats>();
			applyAgilityOffset = 0.5f;
			health = playerStat.current_HP;
			maxHealth = playerStat.current_HP;
		}
		...
		public void Update()
		{
			...
			health = GetComponent<AIBehaviorExamples.PlayerStats>().current_HP;
		}
		...
		public void MoveAgent(Vector3 targetPoint, float targetSpeed, float rotationSpeed, Transform rotationTargetOverride = null)
		{
			...
			if ( isNavAgent && navMeshAgent.enabled )
			{
				...
				navMeshAgent.speed = targetSpeed + (playerStat.playerStat.agility * applyAgilityOffset);
		...
		{
			health -= healthAmount;
			playerStat.HealHP(-(int)healthAmount);
		}
